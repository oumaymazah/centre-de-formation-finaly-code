(function() {
    ...
    // Liste des pages où le script doit s'exécuter
    const allowedPages = [
        '/panier',
    ];
    ...
    // Initialiser le système de réservation sans spinner
    ...
    function initReservationSystem() {
        // Vérifier d'abord le contenu du panier
        checkCartNotEmpty()
            .then(notEmpty => {
                if (!notEmpty) {
                    // Si le panier est vide
                    window.checkingReservations = false; // Important: marquer la fin de la vérification
                    return { hasReservation: false, status: 0, skipReservationCheck: true };
                }
            ...
            .then(result => {
                // Ne traiter la suite que si on n'a pas sauté la vérification des réservations
                if (result.skipReservationCheck) {
                    return; // Sortir ici si le panier était vide
                }
            ...
            .catch(error => {
                ...
                // En cas d'erreur, marquer comme initialisé quand même
                window.buttonsInitialized = true;
            })
            .finally(() => {
    ...
    // 1. MODIFIER LA FONCTION transformReserverButton - Changer la classe du bouton "Voir mes réservations"
    function transformReserverButton(reservationId, reservationStatus = 0, isInitialLoad = false) {
        ...
        // CHANGEMENT ICI : Utiliser une classe différente pour le bouton "Voir mes réservations"
        reserverButton.className = 'voir-reservations-button';
    ...
    // 2. MODIFIER LA FONCTION removeAllButtons - Ajouter la suppression du nouveau bouton
    function removeAllButtons() {
        ...
        // AJOUT ICI : Supprimer aussi le bouton "Voir mes réservations"
        const voirReservationsButton = document.querySelector('.voir-reservations-button');
        if (voirReservationsButton) {
            voirReservationsButton.remove();
        }
    ...
    // 3. MODIFIER LA FONCTION startDOMObserver - Exclure le bouton "Voir mes réservations" des vérifications
        ...
        if (!window.domChangeObserver) {
            window.domChangeObserver = new MutationObserver(function(mutations) {
                ...
                if (document.readyState === 'complete') {
                    ...
                    if (totalPrice && !document.querySelector('.reserver-button') && !document.querySelector('.annuler-button') && !document.querySelector('.voir-reservations-button')) {
                    ...
                    // MODIFICATION ICI : Vérifier directement depuis le DOM plutôt que d'utiliser des Promises
                    ...
                    // CHANGEMENT ICI : Appliquer les restrictions SEULEMENT au bouton de réservation normal (.reserver-button)
                    // PAS au bouton "Voir mes réservations" (.voir-reservations-button)
